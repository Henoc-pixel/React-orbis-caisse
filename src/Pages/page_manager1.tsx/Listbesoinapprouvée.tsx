...
import { Besoin } from "@/Components/types"; // Assurez-vous que ce chemin est correct

const ListBesoinApprouver = () => {
  ...
  const handlePrint = () => {
    ...
    if (printWindow && listRef.current) {
      ...
      <html>
        ...
        <body>
          <h2>Liste des fiches de besoin approuvées</h2>
  ...
  const handleDownloadExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(besoinsApprouves);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "ListeBesoins");
    XLSX.writeFile(workbook, "Liste_Besoins_Approuvés.xlsx");
  };

  const filteredBesoin = besoinsApprouves.filter((item) => {
  ...
  return (
    <Card className="shadow w-100">
      <Card.Header className="text-white bg-primary">
        <h6 className="mb-0">Liste des fiches de besoins approuvés</h6>
      </Card.Header>
      <Card.Body>
